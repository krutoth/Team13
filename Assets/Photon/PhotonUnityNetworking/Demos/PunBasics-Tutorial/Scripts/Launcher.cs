...
#pragma warning disable 649
	...
	public class Launcher : MonoBehaviourPunCallbacks
    {
		...
		[Tooltip("允许用户输入名称、连接和播放的Ui面板")]
		[SerializeField]
		private GameObject controlPanel;

		[Tooltip("向用户通知连接进度的Ui文本")]
		[SerializeField]
		...
		public InputField 房间ID;
		public void 加入特定房间()
        {
			当前操作记录 = "加入特定房间";

			//我们希望确保每次连接时日志都是清晰的，如果连接失败，我们可能会尝试多次失败。
			feedbackText.text = "";
			...
			{
				LogFeedback("加入特定会议室..."+ 房间ID.text);
				Debug.LogError("加入特定会议室结果：" + PhotonNetwork.JoinRoom(房间ID.text));
			}
			else
			{

				LogFeedback("至关重要的是，我们必须首先连接到光子在线服务器。  Connecting...");
		...
		public void 创建房间()
		{
			...
			if (PhotonNetwork.IsConnected)
			{
				LogFeedback("创建会议室..."+ 创建房间ID.text);
				//#关键是，此时我们需要尝试加入随机房间。如果失败，我们将在OnJoinRandomFailed（）中收到通知，并创建一个。
				Debug.LogError("房间创建结果："+ PhotonNetwork.CreateRoom(创建房间ID.text, new RoomOptions { MaxPlayers = this.maxPlayersPerRoom }));
			}
			else
			{
		...
		/// <summary>
		///将反馈记录在玩家的UI视图中，而不是开发者的Unity编辑器中。
		/// </summary>
		...
		public override void OnConnectedToMaster()
		{
			...
			if (isConnecting)
			{
				LogFeedback("OnConnectedToMaster:下一步->尝试加入随机房间");
		...
		public override void OnJoinRandomFailed(short returnCode, string message)
		{
			...
			LogFeedback("OnJoinRandomFailed:下一步->新建房间");
		...
		{
			LogFeedback("<Color=Green>OnJoinedRoom</Color>带有 " + PhotonNetwork.CurrentRoom.PlayerCount+ " 个玩家");
			...
			if (PhotonNetwork.CurrentRoom.PlayerCount == 1)
			{
				...
				PhotonNetwork.LoadLevel("游戏等待玩家阶段");
			}
		}
		#endregion
