...
#pragma warning disable 649
	...
	///处理水平加载（室内同步之外）
	///</摘要>
	public class 联机_竞技模式玩家角色生成 : MonoBehaviourPunCallbacks
    {
		#region 公共领域
		public static 联机_竞技模式玩家角色生成 Instance;
		#endregion
		...
		public List<Transform> 选手生成位置 = new List<Transform>();
		#endregion
		...
		void Start()
		{
			...
			{ //#提示永远不要假设组件的公共属性已正确填充，请始终检查并通知开发人员。
				...
				if (PlayerManager.LocalPlayerInstance==null)
				{
					...
					int 当前游戏玩家处于的排列顺序 = int.Parse(go.GetComponent<PhotonView>().ViewID.ToString().Substring(0, go.GetComponent<PhotonView>().ViewID.ToString().Length - 3));
					go.transform.position = 选手生成位置[当前游戏玩家处于的排列顺序-1].position;
					go.transform.rotation = Quaternion.Euler(0, 90, 0);     //目的角度
