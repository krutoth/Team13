...
namespace Photon.Pun
{
    ...
    public static partial class PhotonNetwork
    {
        ...
        ///<summary>按照PhotonServerSettings文件中的配置连接到光子</摘要>
        ///<备注>
        ///实现IConnectionCallbacks，使您的游戏逻辑了解状态变化。
        ///特别是，IConnectionCallbacks.ConnectedToMasterServer在
        ///客户可以做牵线搭桥。
        ///
        ///此方法将禁用OfflineMode（不会破坏任何实例化的GOs），并且
        ///将IsMessageQueueRunning设置为true。
        ///
        ///光子配置由PUN向导创建，包含AppId，
        ///光子云游戏的区域、服务器地址等。
        ///
        ///要忽略设置文件，请设置相关值并通过调用进行连接
        ///ConnectToMaster，ConnectToRegion。
        ///
        ///若要连接到光子云，设置文件中必须有有效的AppId
        ///（如<a href=“https://dashboard.photonengine.com“>光子云仪表板</a>）。
        ///
        ///连接到光子云可能会失败，原因如下：
        ///-无效的AppId
        ///-网络问题
        ...
        ///-等等。
        ///
        ///通常，请从<see cref=“IConnectionCallbacks.OnDisconnected”/>回调中检出<see cref=“DisconnectCause”/>。
        ///</备注>
...
//#endif
            ...
            Debug.LogError("开始连接 Photon");

            if (PhotonServerSettings == null)
            {
                Debug.LogError("无法连接：加载设置失败。ServerSettings资产必须位于任何“资源”文件夹中，如下所示：" + ServerSettingsFileName);
                return false;
            }
        ...
        public static bool ConnectUsingSettings(AppSettings appSettings, bool startInOfflineMode = false) // parameter name hides static class member
        {
            ...
            {
                Debug.LogWarning("ConnectUsingSettings（）失败。只能在“已断开连接”状态下进行连接。当前状态：" + NetworkingClient.LoadBalancingPeer.PeerState);
                return false;
            }
            if (PhotonHandler.AppQuits)
            {
                Debug.LogWarning("无法连接：应用程序正在关闭。Unity调用了OnApplicationQuit（）。");
                return false;
            }
            if (PhotonServerSettings == null)
            {
        ...
        ///<摘要>
        ///加入与过滤器匹配的随机房间。将回调：OnJoinedRoom或OnJoinRandomFailed。
        ///</摘要>
        ///<备注>
        ///用于随机配对。您可以加入任何房间或在opJoinRandomRoomParams中定义了特定属性的房间。
        ///
        ///如果没有合适的房间或可用的房间（全部已满、已关闭、在另一个大厅或不可见），此操作将失败。
        ///它也可能在实际加入被发现的房间时失败。房间随时可能关闭、爆满或空置。
        ///
        ///此方法只能在客户端连接到主服务器时调用，因此您应该
        ///实现回调OnConnectedToMaster。
        ///检查返回值以确保将在服务器上调用该操作。
        ///注意：如果此方法返回false，则不会有回调。
        ///
        ///更多关于PUN配对的信息：
        ...
        ///https://doc.photonengine.com/en-us/pun/v2/lobby-and-matchmaking/matchmaking-and-lobby
        ///</备注>
        ///<param name=“expectedCustomRoomProperties”>筛选与这些自定义属性（字符串键和值）匹配的房间。若要忽略，请传递null</参数>
        ///<param name=“expectedMaxPlayers”>筛选特定的最大玩家设置。使用0可接受任何maxPlayer值</参数>
        ///＜return＞如果操作已排队并将被发送</返回>
        ...
        ///<param name=“expectedCustomRoomProperties”>筛选与这些自定义属性（字符串键和值）匹配的房间。若要忽略，请传递null</参数>
        ///<param name=“expectedMaxPlayers”>筛选特定的最大玩家设置。使用0可接受任何maxPlayer值</参数>
        ///<param name=“matchingType”>选择一种可用的匹配算法。有关选项，请参见匹配模式枚举</参数>
        ///<param name=“typedLobby”>要查找房间的大厅。传递null，以使用默认大厅。这不会加入该大厅，也不会设置大厅属性</参数>
        ///<param name=“sqlLobbyFilter”>SQL类型游说的筛选器字符串</参数>
        ///<param name=“expectedUsers”>预期加入此游戏的用户以及您希望为其阻止老虎机的用户（按UserId）的可选列表</参数>
        ///＜return＞如果操作已排队并将被发送</返回>
        public static bool JoinRandomRoom(Hashtable expectedCustomRoomProperties, byte expectedMaxPlayers, MatchmakingMode matchingType, TypedLobby typedLobby, string sqlLobbyFilter, string[] expectedUsers = null)
        {
            if (OfflineMode)
            {
                ...
                {
                    Debug.LogError("JoinRandomRoom失败。在脱机模式下，您仍然需要离开一个房间才能进入另一个房间。");
                    return false;
                }
                EnterOfflineRoom("脱机文件室", null, true);
                return true;
            }
            if (NetworkingClient.Server != ServerConnection.MasterServer || !IsConnectedAndReady)
            {
                Debug.LogError("JoinRandomRoom失败。客户端已打开" + NetworkingClient.Server+ "（必须是主服务器才能进行匹配）" + (IsConnectedAndReady ? " 准备好了" : "但尚未做好操作准备（状态：" + NetworkingClient.State + ")") + "。等待回调：OnJoinedLobby或OnConnectedToMaster。");
                return false;
            }
        ...
        ///<摘要>
        ///尝试加入与指定筛选器匹配的文件室，如果未找到文件室，则创建一个文件室。
        ///</摘要>
        ///<备注>
        ///该操作是基于过滤器的随机匹配与创建新房间的选项的组合，
        ///如果没有试衣间的话。
        ///这样做的好处是通过相同的操作创建房间，并且可以找到房间
        ///就在下一个客户，寻找类似的房间。
        ///
        ///有单独的参数用于连接和创建房间。
        ///
        ///此方法只能在连接到主服务器时调用。
        ///此客户端的State设置为ClientState。立即加入。
        ///
        ///调用IMatchmakingCallbacks.On JoinedRoom或IMatchmaking Callbacks.OnCreatedRoom。
        ///
        ///如果在主服务器上创建，则调用IMatchmakingCallbacks.OnJoinRandomFailed。
        ///如果游戏服务器上的“加入”失败，将调用IMatchmakingCallbacks.OnJoinRoomFailed。
        ///
        ///
        ///检查返回值以确保将在服务器上调用该操作。
        ///注意：如果此方法返回false，则不会有回调。
        ///</备注>
        ///＜return＞如果将发送操作（需要连接到主服务器）</返回>
        public static bool JoinRandomOrCreateRoom(Hashtable expectedCustomRoomProperties = null, byte expectedMaxPlayers = 0, MatchmakingMode matchingType = MatchmakingMode.FillRoom, TypedLobby typedLobby = null, string sqlLobbyFilter = null, string roomName = null, RoomOptions roomOptions = null, string[] expectedUsers = null)
        {
            if (OfflineMode)
            {
                if (offlineModeRoom != null)
                {
                    Debug.LogError("JoinRandomOrCreateRoom失败。在脱机模式下，您仍然需要离开一个房间才能进入另一个房间。");
                    return false;
                }
            ...
            if (NetworkingClient.Server != ServerConnection.MasterServer || !IsConnectedAndReady)
            {
                Debug.LogError("JoinRandomOrCreateRoom失败。客户端已打开" + NetworkingClient.Server+ " (必须是主服务器才能进行匹配)" + (IsConnectedAndReady ? " and ready" : " but not ready for operations (State: "+ NetworkingClient.State + ")") + ". Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
                return false;
            }
        ...
        ///<摘要>
        ///创建一个新房间。将回调：OnCreatedRoom和OnJoinedRoom或OnCreateRoomFailed。
        ///</摘要>
        ///<备注>
        ///成功后，这将调用回调OnCreatedRoom和OnJoinedRoom（后者会导致您作为第一个玩家加入）。
        ///在所有错误情况下，都会调用OnCreateRoomFailed。
        ///
        ///如果房间名称已在使用中或RoomOptions发生冲突，则创建房间将失败
        ///彼此之间。检查EnterRoomParams参考中的各种房间创建选项。
        ///
        ///如果您不想创建唯一的房间名称，请将null或“”作为名称，服务器将分配一个房间名称（GUID作为字符串）。
        ///
        ///此方法只能在客户端连接到主服务器时调用，因此您应该
        ///实现回调OnConnectedToMaster。
        ///检查返回值以确保将在服务器上调用该操作。
        ...
        ///https://doc.photonengine.com/en-us/pun/v2/lobby-and-matchmaking/matchmaking-and-lobby
        ///</备注>
        ///<param name=“roomName”>要创建的房间的唯一名称。传递null或“”使服务器生成一个名称</参数>
        ///<param name=“roomOptions”>房间的常用选项，如MaxPlayers、初始自定义房间属性等。请参见RoomOptions类型</参数>
        ///<param name=“typedLobby”>如果为null，则会在当前使用的大厅中自动创建房间（当您没有明确加入大厅时，这是“默认”）</参数>
        ///<param name=“expectedUsers”>预期加入此游戏的用户以及您希望为其阻止老虎机的用户（按UserId）的可选列表</参数>
        ///＜return＞如果操作已排队并将被发送</返回>
        public static bool CreateRoom(string roomName, RoomOptions roomOptions = null, TypedLobby typedLobby = null, string[] expectedUsers = null)
        {
            if (OfflineMode)
            {
                if (offlineModeRoom != null)
                {
                    Debug.LogError("CreateRoom失败。在脱机模式下，您仍然需要离开一个房间才能进入另一个房间。");
                    return false;
                }
            ...
            if (NetworkingClient.Server != ServerConnection.MasterServer || !IsConnectedAndReady)
            {
                Debug.LogError("CreateRoom失败。客户端已打开 " + NetworkingClient.Server + " (must be Master Server for matchmaking)" + (IsConnectedAndReady ? " and ready" : "but not ready for operations (State: " + NetworkingClient.State + ")") + ". Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
                return false;
            }
        ...
        ///<摘要>
        ///按名称连接特定房间，并根据需要创建该房间。将回调：OnJoinedRoom或OnJoinRoomFailed。
        ///</摘要>
        ///<备注>
        ///当玩家设定房间名称以在以下地点会面时，这很有用：
        ///所有参与的客户端调用相同的方法，无论谁是第一个，都会创建房间。
        ///
        ///成功后，客户端将进入指定的房间。
        ///创建房间的客户端将回调OnCreatedRoom和OnJoinedRoom。
        ///加入现有工作室的客户端将只回调OnJoinedRoom。
        ///在所有错误情况下，都会调用OnJoinRoomFailed。
        ///
        ///如果房间已满、已关闭或用户
        ///已存在于房间中（由userId检查）。
        ///
        ///若要返回房间，请使用OpRejoinRoom。
        ///
        ///此方法只能在客户端连接到主服务器时调用，因此您应该
        ///实现回调OnConnectedToMaster。
        ///检查返回值以确保将在服务器上调用该操作。
        ...
        ///
        ///
        ///如果在roomOptions中设置房间属性，则当房间已经存在时，这些属性将被忽略。
        ///这样可以避免延迟加入玩家更改房间属性。
        ///
        ///您可以定义一个expectedUsers数组，以阻止这些用户在房间中的播放器插槽。
        ///Photon中的相应功能被称为“插槽预留”，可以在文档页面中找到。
        ///
        ///
        ///更多关于PUN配对的信息：
        ///https://doc.photonengine.com/en-us/pun/v2/lobby-and-matchmaking/matchmaking-and-lobby
        ///</备注>
        ///<param name=“roomName”>要加入的房间的名称。必须为非null</参数>
        ///<param name=“roomOptions”>房间的选项，以防它还不存在。否则，这些值将被忽略</参数>
        ///<param name=“typedLobby”>要在其中列出新房间的大厅。如果该房间已存在并已加入，则忽略</参数>
        ///<param name=“expectedUsers”>预期加入此游戏的用户以及您希望为其阻止老虎机的用户（按UserId）的可选列表</参数>
        ///＜return＞如果操作已排队并将被发送</返回>
        public static bool JoinOrCreateRoom(string roomName, RoomOptions roomOptions, TypedLobby typedLobby, string[] expectedUsers = null)
        {
            if (OfflineMode)
            {
                if (offlineModeRoom != null)
                {
                    Debug.LogError("JoinOrCreateRoom失败。在脱机模式下，您仍然需要离开一个房间才能进入另一个房间.");
                    return false;
                }
            ...
            if (NetworkingClient.Server != ServerConnection.MasterServer || !IsConnectedAndReady)
            {
                Debug.LogError("JoinOrCreateRoom失败。客户端已打开 " + NetworkingClient.Server + " （必须是主服务器才能进行匹配）" + (IsConnectedAndReady ? " and ready" : "but not ready for operations (State: " + NetworkingClient.State + ")") + ". Wait for callback: OnJoinedLobby or OnConnectedToMaster.");
                return false;
            }
            if (string.IsNullOrEmpty(roomName))
            {
                Debug.LogError("JoinOrCreateRoom失败。房间名称是必需的。如果你不认识一个，你将如何加入？");
                return false;
            }
        ...
        ///<摘要>
        ///按名称连接房间。将回调：OnJoinedRoom或OnJoinRoomFailed。
        ///</摘要>
        ///<备注>
        ///在使用大厅或玩家关注朋友或互相邀请时很有用。
        ///
        ///成功后，客户端将进入指定的房间，并通过OnJoinedRoom进行回调。
        ///在所有错误情况下，都会调用OnJoinRoomFailed。
        ///
        ///如果房间已满、已关闭、不存在或用户
        ///已存在于房间中（由userId检查）。
        ///
        ///若要返回房间，请使用OpRejoinRoom。
        ///当玩家相互邀请，但不清楚谁是第一个回应时，请使用OpJoinOrCreateRoom。
        ///
        ///此方法只能在客户端连接到主服务器时调用，因此您应该
        ///实现回调OnConnectedToMaster。
        ///检查返回值以确保将在服务器上调用该操作。
        ...
        ///<参见cref=“OnJoinedRoom”/>
        ///<param name=“roomName”>要加入的房间的唯一名称</参数>
        ///<param name=“expectedUsers”>预期加入此游戏的用户以及您希望为其阻止老虎机的用户（按UserId）的可选列表</参数>
        ///＜return＞如果操作已排队并将被发送</返回>
        public static bool JoinRoom(string roomName, string[] expectedUsers = null)
        {
            Debug.LogError(OfflineMode);

            if (OfflineMode)
            {
                if (offlineModeRoom != null)
                {
                    Debug.LogError("JoinRoom失败。在脱机模式下，您仍然需要离开一个房间才能进入另一个房间。");
                    return false;
                }
            ...
            if (NetworkingClient.Server != ServerConnection.MasterServer || !IsConnectedAndReady)
            {
                Debug.LogError("JoinRoom失败。客户端已打开" + NetworkingClient.Server + " （必须是主服务器才能进行匹配）" + (IsConnectedAndReady ? "准备好了" : "但尚未做好运行准备（状态: " + NetworkingClient.State + ")") + ". 等待回调：OnJoinedLobby或OnConnectedToMaster。");
                return false;
            }

            if (string.IsNullOrEmpty(roomName))
            {
                Debug.LogError("JoinRoom失败。房间名称是必需的。如果你不认识一个，你将如何加入？");
                return false;
            }
        ...
        ///<摘要>
        ///按roomName重新加入房间（在内部使用userID返回）。将回调：OnJoinedRoom或OnJoinRoomFailed。
        ///</摘要>
        ///<备注>
        ///失去连接后，您可以返回房间继续玩游戏，
        ///如果客户端重新连接的速度足够快。使用Reconnect（）和此方法。
        ///缓存您所在的房间名称，然后使用RejoinRoom（房间名称）返回游戏。
        ///
        ///注意：为了能够重新加入任何房间，您需要使用UserID！
        ///您还需要设置RoomOptions.PlayerTtl。
        ///
        ///<b>重要提示：Instantiate（）和RPC的使用还不受支持</b>
        ///如果使用光子视图，则光子视图的所有权规则会阻止无缝返回游戏。
        ///请改用带有事件缓存的自定义属性和RaiseEvent。
        ///
        ///常见用例：按下iOS设备上的锁定按钮，您会立即断开连接。
        ///
        ///复联室不会发送任何玩家属性。相反，客户端将从服务器接收最新的信息。
        ///如果你想设置新的玩家属性，请在重新加入后进行设置。
        ///</备注>
        public static bool RejoinRoom(string roomName)
        {
            if (OfflineMode)
            {
                Debug.LogError("由于脱机模式，RejectRoom失败。");
                return false;
            }
            if (NetworkingClient.Server != ServerConnection.MasterServer || !IsConnectedAndReady)
            {
                Debug.LogError("复辩室失败。客户端已打开" + NetworkingClient.Server + " (必须是主服务器才能进行匹配)" + (IsConnectedAndReady ? " 准备好了" : "但尚未做好操作准备（状态：" + NetworkingClient.State + ")") + ". 等待回调：OnJoinedLobby或OnConnectedToMaster。");
                return false;
            }
            if (string.IsNullOrEmpty(roomName))
            {
                Debug.LogError("复辩室失败。房间名称是必需的。如果你不认识一个，你将如何加入？");
                return false;
            }
        ...
        ///<summary>当客户端在游戏过程中失去连接时，此方法会尝试重新连接并重新加入房间</摘要>
        ///<备注>
        ///这种方法直接重新连接到PUN之前所在房间的游戏服务器。
        ///如果房间在此期间关闭，PUN将调用OnJoinRoomFailed并将此客户端返回到主服务器。
        ///
        ///如果此客户端将尝试重新连接并重新加入（如果满足条件），请检查返回值。
        ///如果ReconnectAndRejoin返回false，您仍然可以尝试重新连接并重新加入。
        ///
        ///与PhotonNetwork.RejoinRoom类似，这要求每个玩家使用唯一的ID（UserID）。
        ///
        ///复联室不会发送任何玩家属性。相反，客户端将从服务器接收最新的信息。
        ///如果你想设置新的玩家属性，请在重新加入后进行设置。
        ///</备注>
        ///＜return＞如果没有已知的房间或游戏服务器可返回，则为False。然后，此客户端不会尝试重新连接和重新连接</返回>
        ...
        ///<summary>离开当前文件室，返回主服务器，在那里您可以加入或创建文件室（请参阅备注）</摘要>
        ///<备注>
        ///这将使用PhotonView清理所有（网络）游戏对象，除非您将autoCleanUp更改为false。
        ///返回到主服务器。
        ///
        ///在离线模式下，本地“假”房间会被清理干净，OnLeftRoom会立即被调用。
        ///
        ///在一个有播放器的房间里TTL&lt；0，LeaveRoom只会使客户端处于非活动状态。玩家停留在房间的玩家列表中
        ///并且可以稍后返回。故意将“非活动”设置为false，意味着“放弃”房间，尽管
        ///播放器TTL允许您回来。
        ///
        ///在playerTTL==0的房间中，变为非活动状态没有任何效果（客户端会立即从房间中移除）。
        ///</备注>
        ///<param name=“becomeInactive”>如果此客户端在具有playerTTL&lt；0。默认为true</参数>
        ...
        ///<摘要>
        ///内部使用helper方法设置一个离线房间，为actor和master客户端提供数字并进行回调。
        ///</摘要>
