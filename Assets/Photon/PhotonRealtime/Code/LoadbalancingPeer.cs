...
namespace Photon.Realtime
{
    ...
    public class LoadBalancingPeer : PhotonPeer
    {
        ...
        ///<摘要>
        ///按名称连接房间，或者在不存在指定名称的房间的情况下创建新房间。
        ///OperationResponse取决于对等方连接到的服务器：
        ///Master将返回一个要连接的游戏服务器。
        ///游戏服务器将返回加入房间的数据。
        ///这是一个异步请求，用于触发OnOperationResponse（）调用。
        ///</摘要>
        ///<备注>
        ///如果房间不存在（不再存在），OperationResponse的returnCode将为ErrorCode.GameDoesNotExist。
        ///其他可能的错误代码有：GameClosed，GameFull。
        ///</备注>
        ///＜return＞如果可以发送操作（需要连接）</返回>
        public virtual bool OpJoinRoom(EnterRoomParams opParams)
        {
            ...
            {
                op[ParameterCode.JoinMode] = (byte)JoinMode.RejoinOnly; // 从JoinMode.Join或ReCoin更改
            }
