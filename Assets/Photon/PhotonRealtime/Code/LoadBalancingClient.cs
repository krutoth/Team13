...
namespace Photon.Realtime
{
    ...
    public class LoadBalancingClient : IPhotonPeerListener
    {
        ...
        /// <summary>Name Server Host Name for Photon Cloud. Without port and without any prefix.默认 ns.photonengine.io</summary>
        ...
        ///
        ///房间的名称必须是唯一的（根据地区、appid和游戏版本）。
        ///当您的标题使用全局匹配或邀请（例如外部解决方案）时，
        ///要加入一个房间，请记住地区和游戏版本。
        ///
        ///
        ///此客户端的State设置为ClientState。当操作可以
        ///被调用。在后台，客户端将切换服务器并调用各种相关操作。
        ///
        ///当您在会议室中时，此客户端的状态将变为ClientState.Joined。
        ///
        ///
        ///进入文件室时，此客户端的播放机自定义属性将发送到文件室。
        ///使用LocalPlayer.SetCustomProperties设置它们，即使还不在房间中。
        ///请注意，玩家属性将被缓存在本地，离开房间时不会被擦除。
        ///
        ///你可以定义一个expectedUsers数组，为朋友或聚会成员在房间里保留玩家位置。
        ///Photon中的相应功能被称为“插槽预留”，可以在文档页面中找到。
        ///</备注>
        ///<param name=“enterRoomParams”>要加入的房间的属性定义</参数>
        ///＜return＞如果当前可以发送操作（需要连接到主服务器）</返回>
        public bool OpJoinRoom(EnterRoomParams enterRoomParams)
        {
            Debug.LogError("OpJoinRoom");
